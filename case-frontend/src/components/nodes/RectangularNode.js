import "../../styles/nodeStyles/rectangularNodeStyle.css";
import { IconButton } from "@mui/material";
import CloseIcon from "@mui/icons-material/Close";
import { Handle, NodeResizer, Position, useReactFlow } from "@xyflow/react";
import { memo, useCallback, useState } from "react";

function RectangularNode({ id, data }) {
  const [editMode, setEditMode] = useState(false);
  const [title, setTitle] = useState(data?.label || "Process");
  ...
  return (
    <div
      className="rectangular-node-layout"
      onPointerEnter={() => setDeleteVisible(true)}
      onPointerLeave={() => setDeleteVisible(false)}
    >
      <NodeResizer minWidth={100} minHeight={50} color="black" keepAspectRatio />
      ...
      <div className="rectangular-node-content">
        {editMode ? (
          <input
            id="title"
            name="title"
            ...
            value={title}
            className="nodrag rectangular-node-input"
            autoFocus
          />
        ) : (
